...
def get_billing_config(lock_for_update=False):
    config = BillingConfig.objects.first()
    if config is None:
        config = BillingConfig.objects.create()

    queryset = BillingConfig.objects
    if lock_for_update:
        queryset = queryset.select_for_update()
    return queryset.get(pk=config.pk)


def build_billing_summary():
    config = get_billing_config()
    transactions = UsageTransaction.objects.all()[:50]
    ...
    return {
        ...
        "usage": config.usage,
        "limit": config.limit,
        "debit_amount": config.debit_amount,
        ...
        "projected_create_usage": config.usage + config.debit_amount,
        "projected_edit_usage": config.usage + config.edit_amount,
        "can_fetch_rc": (config.usage + config.fetch_amount) <= config.limit,
        "can_create_rc": (config.usage + config.debit_amount) <= config.limit,
...
def apply_usage_charge(config, amount, transaction_type, reg_number=None, note=""):
