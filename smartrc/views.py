...
def handle_rc_create(raw_data, explicit_rc_type=None):
    ...
    with db_transaction.atomic():
        config = get_billing_config(lock_for_update=True)
        amount = config.edit_amount if existing_instance else config.debit_amount
        ...
        action_note = f"{normalized_type} RC updated" if existing_instance else f"{normalized_type} RC created"
    ...
    def put(self, request, rc_type, reg_number, *args, **kwargs):
        ...
        with db_transaction.atomic():
            ...
            apply_usage_charge(
                ...
                reg_number=reg_number,
                note=f"{normalized_type} RC edited",
            )
...
def save_rc_details(request):
    ...
    try:
        ...
        reg_number = data.get("regnNo")

        if not reg_number:
            return JsonResponse(
                {
                    "success": False,
                    "error": "Registration number (regnNo) is required.",
                },
                status=400,
            )

        _, created = Rc.objects.update_or_create(
            reg_number=reg_number,
            defaults={"data": data},
        )

        message = (
            "RC details created successfully."
            if created
            else "RC details updated successfully."
        )
